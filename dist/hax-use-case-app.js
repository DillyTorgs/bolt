...
class HaxUseCaseCard extends r {
  ...
  render() {
    const { id, title, description, image, tags, demoUrl } = this.useCase;
...
class HaxFilterSidebar extends r {
  static get properties() {
    return {
    ...
    return x`
      <div class="filters">
        ...
        <div class="tag-filter">
          ${this.availableTags.map(tag => x`
            <button 
              class="tag-button ${this.activeFilters.includes(tag) ? 'active' : ''}"
              @click=${() => this.dispatchEvent(new CustomEvent('toggle-filter', { detail: tag }))}
            >
              ${tag}
            </button>
          `)}
        ...
        <button 
          class="reset-button" 
          @click=${() => this.dispatchEvent(new CustomEvent('reset-filters'))}
        >
          Reset Filters
        </button>
      </div>
...
class HaxUseCaseApp extends r {
  ...
  handleSelectUseCase(e) {
    const useCaseId = e.detail;
    this.activeUseCase = this.activeUseCase === useCaseId ? null : useCaseId;
    console.log('Active Use Case:', this.activeUseCase);
  }
    ...
    return x`
      <div class="container">
        <hax-filter-sidebar
          ...
          @toggle-filter=${this.handleToggleFilter}
          @reset-filters=${this.handleResetFilters}
        ></hax-filter-sidebar>
        ...
        <div>
          ...
          <div class="cards-container">
            ${this.getFilteredUseCases().map(useCase => x`
              <hax-use-case-card
                .useCase=${useCase}
                ?selected=${this.activeUseCase === useCase.id}
                @select-use-case=${this.handleSelectUseCase}
              ></hax-use-case-card>
            `)}
          </div>
        </div>
